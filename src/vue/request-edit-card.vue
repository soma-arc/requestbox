<template>
  <div class="card medium" v-bind:class="{ deadlineWarning: closedToDeadline }">
    ...
    <label class="button" v-bind:for="request.id" @click="editStart">Edit</label>
    ...
    <div class="modal" id="mod">
      <div class="card large modal-card">
        <div class="section">
          ...
          <label class="button" v-bind:for="request.id" @click="save">変更を保存</label>
        </div>
        <div id="edit-contents">
          ...
          <transition name="component-fade" mode="out-in">
            <input v-show="request.format === 'other'" v-model="formatOther" :required="true">
          </transition>
...
export default {
    ...
    methods: {
        addDate: function(event) {
            this.request.dateOfExperiment.push(new Date());
        },
        save: function(event) {
            console.log(this.request);
            this.request.updateDBData();
        },
